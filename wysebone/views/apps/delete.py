...
@permission_required('wysebone.delete_app', raise_exception=True)
def DeleteAppView(request, app_code):
    ...
    try:
        ...
        messages.error(request, _("%(name)s with ID \"%(key)s\" doesn't exist. Perhaps it was deleted?") % {'name': _('Applications').title(), 'key': app_code})
    ...
    if request.POST.get('post'):
        ...
        app_info.save()
        messages.success(request, _("The %(name)s \"%(obj)s\" was deleted successfully.") % {'name': _('Applications').title(), 'obj': app_info.name})
