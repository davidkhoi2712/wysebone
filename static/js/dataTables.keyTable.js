...
$.extend( KeyTable.prototype, {
    ...
    /**
	 * Get an array of the column indexes that KeyTable can operate on. This
	 * is a merge of the user supplied rows and the visible rows.
	 *
	 * @private
	 */
	_rows: function ()
	{
		var dt = this.s.dt;
		var out = dt.rows( this.c.rows ).indexes();
	...
	_focus: function ( row, column, shift, originalEvent )
	{
        ...
        // In the available rows?
		if ( $.inArray( row, this._rows() ) === -1 ) {
			return;
		}
		...
		$('.row-num', dt.table().container()).removeClass('wysebone-primary');
		$('.column-name', dt.table().container()).removeClass('wysebone-primary');
		$('[data-y="' + row+'"]', dt.table().container()).find('.row-num').addClass('wysebone-primary');
		$(cell.column(column).header()).addClass('wysebone-primary');
...
KeyTable.defaults = {
    ...
    /**
	 * Rows that can be focused. This is automatically merged with the
	 * visible rows as only visible rows can gain focus.
	 * @type {String}
	 */
	rows: '', // all
