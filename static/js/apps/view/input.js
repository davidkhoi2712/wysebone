function InOutput(input_type, option) {
    ...
    /**
     * Create input, output to display
     * @version 1.0.1
     */
    this.create = function(index, obj) {
        ...
        if (self.type == 'input') {
            ...
            _.forEach(self.field_code, function(item) {
                ...
                options += '<option' + selected + '>' + _.escape(item) + '</option>';
            });
    ...
    this.init = function() {
        ...
        self.field_code = _.union(option.parent.items_management.get_list_field_code(option.field_code), [option.field_code]);
